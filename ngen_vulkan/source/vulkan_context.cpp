...
namespace ngen::vulkan {
    ...
    VkPhysicalDevice VulkanContext::selectDevice() {
        PhysicalDevice::enumerate(m_physicalDevices, m_instance);
    ...
    VkDevice VulkanContext::createDevice(const PhysicalDevice &physicalDevice) {
    ...
    int VulkanContext::findQueueFamily(const PhysicalDevice &device, VkQueueFlags flags) const {
    ...
    //! \param flags [in] - The flags of the particular queue we are to scan for.
    //! \returns Index of the requested queue within the device or -1 if it could not be found.
    int VulkanContext::findPresentationQueue(const PhysicalDevice &device, WindowSurface &surface) const {
        uint32_t queueFamilyCount = 0;
        vkGetPhysicalDeviceQueueFamilyProperties(device, &queueFamilyCount, nullptr);
